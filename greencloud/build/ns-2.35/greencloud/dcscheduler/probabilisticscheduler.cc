...
TskComAgent* ProbabilisticScheduler::scheduleTask(CloudTask* task, std::vector<ResourceProvider* > providers){
	...
	std::vector <ResourceProvider*>::iterator res_p;
	std::vector <double>::iterator score;
	std::vector <double>::iterator cumul;
...
//	std::cerr << "Random: " << r  << "\n";
	std::vector <double>::iterator lb = lower_bound(mdens_score_cumulative.begin(),mdens_score_cumulative.end(),r);
