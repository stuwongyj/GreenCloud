...
#include "dcscheduler/temperaturescheduler.h"
...
protected:
	std::vector <ResourceProvider*> host_list;
	std::vector <TskComAgent*> host_agent_list;
	std::vector <ResourceProvider*> vm_list;
	std::vector <TskComAgent*> vm_agent_list;
	std::vector <PowerModel*> power_model_list;

	DcScheduler* dcScheduler;

	/* Two dimensional structure: std::vector of resource types, that are itself std::vectors of specification for given type.
	 * this structure contains all resource specifications available in data center.*/
	std::vector < ResourceSpec* > resource_specification_list;
	std::vector < ResourceSpec* > virt_resource_specification_list;
