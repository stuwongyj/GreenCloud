...
void DataCenter::clear(){
	...
	std::vector <ResourceSpec*>::iterator rs;
...
int DataCenter::setScheduler(const char * scheduler_name){
	...
	if(strcmp(scheduler_name, "Green") == 0){
		...
		return 1;
	} else if(strcmp(scheduler_name, "Temperature") == 0){
		dcScheduler = new TemperatureScheduler();
		return 1;
	}
...
void DataCenter::printResourceSpecs(){
	std::vector <ResourceSpec*>::iterator iter;
